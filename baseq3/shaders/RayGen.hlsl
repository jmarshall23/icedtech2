...
[shader("raygeneration")] void RayGen() {
	...
	// Fog
	if(hit.colorAndDistance.w != -1 && fogInfo.z != -1)
	{
		float3 viewPos = float3(timeViewOrg.y, timeViewOrg.z, timeViewOrg.w);		
		if(fogInfo.z == 1) {
			float fog = getFogFactor(length(hit.worldOrigin.xyz - viewPos), 400, 50);
			gLightOutput[launchIndex].xyz = lerp(gLightOutput[launchIndex].xyz, float3(0.05, 0.05, 0.05), fog);
		}
		else if(fogInfo.z == 2) {
			float fog = getFogFactor(length(hit.worldOrigin.xyz - viewPos), 400, 50);
			gLightOutput[launchIndex].xyz = lerp(gLightOutput[launchIndex].xyz, float3(1.0, 0.0, 0.0), fog);
		}
		else if(fogInfo.z == 3) {
			float fog = getFogFactor(length(hit.worldOrigin.xyz - viewPos), 400, 50);
			gLightOutput[launchIndex].xyz = lerp(gLightOutput[launchIndex].xyz, float3(0.0, 1.0, 0.0), fog);
		}
		else {
			float fog = getFogFactor(length(hit.worldOrigin.xyz - viewPos), 1000, 600);
